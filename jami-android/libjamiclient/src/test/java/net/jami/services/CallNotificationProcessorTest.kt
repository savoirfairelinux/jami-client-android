...
class CallNotificationProcessorTest {
    ...
    @Test
    fun eagerRemovalConstructionFailureStillRunsCleanupHistoryAndFollowingCalls() {
        val failure = IllegalStateException("notification account lookup")
        val terminal = call().apply { setCallState(CallStatus.OVER) }
        val completed = mutableListOf<String>()
        val observer = processor.process { update ->
            if (update === terminal) {
                val notification = processor.prepareRemoval(update) { throw failure }
                completed.add("state-cleanup")
                notification.andThen(Completable.fromAction { completed.add("history") })
            } else Completable.fromAction { completed.add("next-call") }
        }.test()

        updates.onNext(terminal)
        updates.onNext(call("next"))
        updates.onComplete()

        assertEquals(listOf("state-cleanup", "history", "next-call"), completed)
        assertSame(failure, errors.single())
        observer.assertComplete().assertNoErrors()
    }

    @Test
    fun removalCapturesParticipantMetadataBeforeCleanupAndRunsOnlyWhenSubscribed() {
        val participants = mutableListOf("peer")
        var captured: List<String> = emptyList()
        var executed = false
        val notification = processor.prepareRemoval(call()) {
            captured = participants.toList()
