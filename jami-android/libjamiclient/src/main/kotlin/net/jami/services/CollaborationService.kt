...
class CollaborationService(private val executor: ExecutorService) {
    ...
    fun updatesFor(accountId: String, conversation: Uri, documentId: String): Observable<ByteArray> =
        documentUpdates
            ...
            // An empty payload only says that a closed replica changed. It has no
            // CRDT update for an open editor to merge.
            .filter { it.update.isNotEmpty() }
            .map { it.update }
    ...
    fun documents(accountId: String, conversation: Uri): Single<List<CollaborativeDocument>> =
        fromDaemon {
            ...
            try {
                vect.mapNotNull { CollaborativeDocument.fromNative(it) }
            } finally {
                vect.delete()
            }
        }
