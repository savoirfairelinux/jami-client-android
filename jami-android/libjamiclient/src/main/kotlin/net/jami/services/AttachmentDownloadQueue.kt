 ...
 * reports their progress through [update] and their end through [finish]. A
 * download leaves the in-flight set once complete, or once it has not
 * progressed for [STALL_TIMEOUT_NANOS]; in the latter case the daemon still
 * holds the request and retries it on the next sync with the device, so
 * nothing is lost.
 *
...
class AttachmentDownloadQueue {
    ...
    /** Releases the slot of a download the daemon reported as over (finished, failed or canceled), whatever its progress. */
    @Synchronized
