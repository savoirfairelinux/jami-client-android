...
abstract class CallService(
    ...
    fun makeCallObservable(accountId: String, conversationUri: Uri?, number: Uri, hasVideo: Boolean): Observable<Call> =
        makeCall(accountId, conversationUri, number, hasVideo)
            .flatMapObservable { call: Call -> getCallUpdates(call) }

    fun makeCallIfAllowed(account: String, conversationUri: Uri?, number: Uri, hasVideo: Boolean): Single<Call> =
        requestMakeCall(account, conversationUri, number.rawUriString, hasVideo)
            .onErrorReturnItem(CALL_ALLOWED_VAL)
            .flatMap { result ->
                if (!result.allowed)
                    Single.error(SecurityException())
                else
                    makeCall(account, conversationUri, number, hasVideo)
                        .doOnSuccess { result.setCall(it) }
            }

    private fun makeCall(account: String, conversationUri: Uri?, number: Uri, hasVideo: Boolean): Single<Call> =
        Single.fromCallable<Call> {
            Log.i(TAG, "makeCall() thread running… $number hasVideo: $hasVideo")
            val media = VectMap()
            media.reserve(if (hasVideo) 2 else 1)
            media.add(Media.DEFAULT_AUDIO.toMap())
            if (hasVideo)
                media.add(Media.DEFAULT_VIDEO.toMap())
            val callId = JamiService.makeCallWithMedia(account, number.uri, media)
