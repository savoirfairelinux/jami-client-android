...
class CallService(
    ...
    fun requestVideoMediaChange(conf: Conference, uri: String?) {
        Log.d(TAG, "ASDF hasVideo:${conf.hasVideo()} videoMuted:${conf.isVideoMuted} isConf:${conf.isConference} uri:$uri")
        ...
        proposedMediaList.forEach{
            Log.d(TAG, "ASDF label:${it.label} source:${it.source} type:${it.mediaType} enabled:${it.isEnabled} muted:${it.isMuted} onHold:${it.isOnHold}")
        }
