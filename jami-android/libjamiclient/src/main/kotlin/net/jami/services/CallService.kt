...
abstract class CallService(
    ...
    fun hostConference(
        ...
        Log.i("devdebug", "hostConference() account=$account conversationUri=$conversationUri numberUri=$numberUri hasVideo=$hasVideo")
        ...
        return conferenceSubject.firstOrError().doOnSubscribe {
            Log.w("devdebug", "hostConference() conferenceSubject.firstOrError()")
            mExecutor.execute {
                JamiService.placeCallWithMedia(account, numberUri.uri, mediaMap)
            }
            Log.w("devdebug", "hostConference() doOnSubscribe done")
        }
    }
    ...
    fun conferenceCreated(accountId: String, conversationId: String, confId: String) {
        Log.d("devdebug", "callservice.kt conferencecreated: $confId")
        ...
        Log.w("devdebug", "callservice.kt conferenceCreated: $confId $participants")
        conferenceSubject.onNext(conf)
    }
