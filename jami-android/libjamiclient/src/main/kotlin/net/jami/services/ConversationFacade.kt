...
//            }
    ...
    private fun handleDataTransferEvent(transfer: DataTransfer) {
        ...
        // Attachments synchronized after a device import are old messages: no notification
        // for them, up to and including the event that reports the file as received.
        if (mAccountService.isSyncingAttachment(transfer.fileId)) {
            if (status == TransferStatus.TRANSFER_FINISHED)
                mAccountService.attachmentSynced(transfer.fileId!!)
            return
        }
