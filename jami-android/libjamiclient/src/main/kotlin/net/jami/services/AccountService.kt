        ...
        val interaction: Interaction = when (type) {
            ...
            "application/data-transfer+json" -> {
                try {
                    ...
                    // Fallback: for outgoing files (or when daemon reports incomplete),
                    // check the client's own storage path. The file may exist there even
                    // when the daemon's symlink/info is stale (e.g., after app restart
                    // race condition or broken symlink).
                    if (!isComplete && !fileId.isNullOrEmpty()) {
