...
class AccountService(
    ...
    // Captured the first time a task runs on mExecutor; used to guard against
    // deadlock in setAccountsActive(awaitCompletion=true).
    @Volatile private var mExecutorThread: Thread? = null

    init {
        // Eagerly capture the executor thread so the deadlock guard in
        // setAccountsActive(awaitCompletion=true) is reliable even on the
        // very first call, before any other task has run.
        mExecutor.execute { mExecutorThread = Thread.currentThread() }
    }
    /**
     ...
     * @return true if the change was applied successfully, false on timeout or interrupt
     */
    fun setAccountsActive(active: Boolean, forceAll: Boolean = false, awaitCompletion: Boolean = false): Boolean {
        if (awaitCompletion) {
            check(Thread.currentThread() !== mExecutorThread) {
                "setAccountsActive(awaitCompletion=true) must not be called from the daemon executor thread — would deadlock"
            }
        }
        val latch = if (awaitCompletion) java.util.concurrent.CountDownLatch(1) else null
        mExecutor.execute {
            if (mExecutorThread == null) mExecutorThread = Thread.currentThread()
            try {
