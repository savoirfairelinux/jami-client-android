...
class AccountService(
    ...
    fun setAccountsActive(active: Boolean, forceAll: Boolean = false, awaitCompletion: Boolean = false) {
        // Refuse to block on the daemon executor itself — that would self-deadlock.
        val safeAwait = if (awaitCompletion && Thread.currentThread().name == "DRing") {
            Log.w(TAG, "setAccountsActive: awaitCompletion ignored, caller is on daemon thread")
            false
        } else awaitCompletion
        val latch = if (safeAwait) java.util.concurrent.CountDownLatch(1) else null
        mExecutor.execute {
            try {
                ...
                for (a in mAccountList) {
                    ...
                    if (a.isDhtProxyEnabled && !forceAll) {
                        ...
                        // Shut down P2P connections when forcing all accounts off
                        // (idle/background shutdown). Requires daemon patch #34036.
                        val shutdownConnections = forceAll && !active
                        JamiService.setAccountActive(a.accountId, active, shutdownConnections)
                    }
                }
            } finally {
                latch?.countDown()
