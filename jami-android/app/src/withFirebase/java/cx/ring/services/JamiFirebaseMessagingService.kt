...
class JamiFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        if (!appInForeground && app != null) {
            // Positive call signal from the protocol-guaranteed pt field (the daemon
            // always labels call connection requests audioCall/videoCall), plus a
            // defensive fallback: a high-priority push lacking pt entirely is treated
            // as call-like, so a proxy failing to copy the type cannot downgrade a call
            // wakeup. Pushes with an explicit non-call pt (sip, git…) stay normal: DHT
            // values default to high priority, and reconnecting on every message push
            // is exactly what this gate avoids.
            val isCallPush = isCallNotification || isDhtProxyCallWakeup(remoteMessage)
                    || (remoteMessage.data["pt"] == null
