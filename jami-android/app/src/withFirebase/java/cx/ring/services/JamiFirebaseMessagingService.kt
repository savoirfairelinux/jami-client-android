...
class JamiFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        // Restore/reconnect is gated on the same push-availability condition as the
        // deactivation runnable: if push support was disabled while backgrounded, a
        // stale FCM delivery must not reactivate accounts that the follow-up
        // deactivation would then refuse to deactivate, leaving them active forever.
        if (!appInForeground
            && app != null
            && app.mPreferencesService.settings.enablePushNotifications
            && app.pushToken != null
        ) {
