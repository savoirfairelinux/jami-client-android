...
class JamiFirebaseMessagingService : FirebaseMessagingService() {
    ...
    private fun classifyWakeup(remoteMessage: RemoteMessage): PushWakeup {
        ...
        pushTypes.splitToSequence(',').forEachIndexed { i, rawType ->
            ...
            val isNew = if (id.isNullOrEmpty()) {
                ...
                // Separate per-kind caches: message volume must never evict call dedupe
                // state, so an evicted call id can't be misread as a new call on a stale
                // re-delivery (see seenCallIds/seenMessageIds). Picking the cache by kind
                // also keeps the two id domains disjoint.
    ...
    companion object {
        ...
        // (catch-up dumps re-push stored values on every fresh server listener). Kept in
        // two separate bounded LRU caches so that frequent message ids cannot evict call
        // dedupe state: messages and calls are announced on the same device key, and a
        // busy conversation produces far more commits than calls — a shared cache would
        // let message volume push out recent call ids within the ~10 min DHT retention,
        // and a re-delivered stale call value whose id was evicted would then be treated
        // as a new incoming call again (foreground service, restore, reconnect), the
        // exact churn this cache prevents. Calls are rare, so 256 is ample; the message
        // cache is larger to absorb conversation bursts. Companion-level: the FCM
        // service instance can be recreated between pushes within the same process.
