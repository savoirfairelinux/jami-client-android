...
class JamiFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        serviceScope.launch {
            try {
                if (app != null) {
                    // Gate dispatch on successful bootstrap: if the daemon
                    // failed to start within the timeout, dispatching anyway
                    // would silently drop the push.
                    if (!app.ensureDaemonStarted()) {
                        Log.e(TAG, "onMessageReceived: daemon failed to start within timeout, push may be lost")
                        return@launch
                    }
