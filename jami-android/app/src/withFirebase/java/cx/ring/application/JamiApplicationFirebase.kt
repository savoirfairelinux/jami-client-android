...
class JamiApplicationFirebase : JamiApplication() {
    ...
    fun notePushReceived(isCallPush: Boolean) {
        // Capture the receipt time now, but apply the update on the main handler so all
        // lifecycle/deactivation state stays on a single thread. The pending deactivation
        // is cancelled and re-armed in the same operation: a runnable scheduled before
        // this push may already be due and would otherwise run with stale timestamps,
        // queueing account deactivation right after the FCM path restored them.
        val now = SystemClock.elapsedRealtime()
        backgroundHandler.post {
            lastPushTime = now
            if (isCallPush) lastCallPushTime = now
            // Cancels any pending (possibly already-due) runnable and re-schedules.
