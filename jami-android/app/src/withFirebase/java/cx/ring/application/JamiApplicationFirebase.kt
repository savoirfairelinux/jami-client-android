...
class JamiApplicationFirebase : JamiApplication() {
    ...
    private val deactivateRunnable = Runnable {
        ...
        when {
            ...
            // deactivated once the call ends even if no lifecycle event fires.
            mCallService.hasActiveCalls() -> scheduleBackgroundDeactivation()
    ...
    fun notePushReceived(isCallPush: Boolean) {
        // Capture the receipt time now, but update the fields on the main handler so all
        // lifecycle/deactivation state stays on a single thread. The deactivation runnable
        // always runs with a delay, so this post is processed before any evaluation.
        val now = SystemClock.elapsedRealtime()
    ...
    override fun onCreate() {
        ...
        registerActivityLifecycleCallbacks(object : ActivityLifecycleCallbacks {
            override fun onActivityStarted(activity: Activity) {
                ...
                backgroundHandler.removeCallbacks(deactivateRunnable)
                // Not gated on current network state: connectivity may be reported stale
                // right after waking, and activating an account without network is harmless
                // (the daemon reconnects when connectivity events arrive). Gating could
                // leave accounts deactivated with no later event to reactivate them.
                if (startedActivityCount++ == 0
                    && mPreferencesService.settings.enablePushNotifications
                    && pushToken != null
                ) {
