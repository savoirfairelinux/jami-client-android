...
class JamiApplicationFirebase : JamiApplication() {
    ...
    private val deactivateRunnable = Runnable {
        ...
        val graceRemaining = maxOf(
            callGraceRemaining,
            lastPushTime.get() + PUSH_GRACE_MS - now
        )
        ...
        when {
            ...
            // A call push is being negotiated: hasActiveCalls() stays false until the
            // connection request reaches CallService (tens of seconds through a cold
            // proxy reconnection), so a call push arriving after the cap elapsed would
            // otherwise be deactivated mid-negotiation — the exact failure this
            // optimization must never cause. The call grace window is therefore exempt
            // from the cap, extending the episode by at most CALL_PUSH_GRACE_MS past
            // its bound per call push.
