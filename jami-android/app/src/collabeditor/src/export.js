...
export const ATTACHMENT_SCHEME = 'jami-attachment:'
...
function imageUrl(run) {
    return ATTACHMENT_SCHEME + encodeURIComponent(run.image.id || '')
}

function htmlRun(run) {
    if (run.image) {
        const width = run.width > 0 ? ` width="${run.width}"` : ''
        return `<img src="${escapeHtml(imageUrl(run))}"${width}>`
    }
...
function markdownRun(run) {
    if (run.image) return `![](${imageUrl(run)})`
    let out = escapeMarkdown(run.text)
    const attrs = run.attributes
...
function toMarkdown(lines) {
    ...
    let ordinal = 0
    for (const line of lines) {
        const body = line.runs.map(markdownRun).join('')
 ...
 * @return { text, attachments } where attachments lists, once each and in the
 *         order they appear, the ids left as `jami-attachment:<id>`; or null if
 *         the format is not one of "html", "md" or "txt".
 */
export function exportDocument(delta, format, title, without) {
    ...
    const lines = linesOf(delta).map((line) => ({
        ...
        runs: line.runs.filter((run) => !run.image || !dropped.includes(run.image.id || '')),
    }))
    const attachments = []
    if (format !== 'txt') {
        for (const line of lines)
            for (const run of line.runs) {
                ...
                const id = run.image.id || ''
                if (id && !attachments.includes(id)) attachments.push(id)
            }
    }
    return { text: write(lines, title), attachments }
}
