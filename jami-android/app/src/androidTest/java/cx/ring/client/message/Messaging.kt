...
class Messaging {
    ...
    companion object {
        ...
        const val TEST_MESSAGE_5 = "my colored message peer"
    ...
    // Todo: Is the color changed in the peer side?
    // @Test
    fun a12_changeConversationColor_changedInPeerSide() {
        // Move to accountA.
        AccountNavigationUtils.moveToAccount(accountA.displayUri!!)
        ...
        mActivityScenarioRule.scenario.onActivity {
            it.mAccountService.sendConversationMessage(
                ...
                conversationUri = accountA.getConversations().first().uri,
                txt = TEST_MESSAGE_5,
                replyTo = null
            )
        }
        ...
        onView(withId(R.id.bubble_message_text))
            .perform(waitUntil(allOf(withText(TEST_MESSAGE_5), isDisplayed())))

        // Check if the conversation color is changed.
        onView(allOf(withId(R.id.message_content), hasDescendant(withText(TEST_MESSAGE_5))))
            .check(matches(hasBackgroundColor(R.color.conversation_palette_red)))
    }
    ...
    // Todo: Is the symbol change for the peer side?
    // @Test
    fun a14_changeConversationSymbol_changedInPeerSide() {
        // Move to accountA.
        AccountNavigationUtils.moveToAccount(accountA.displayUri!!)
