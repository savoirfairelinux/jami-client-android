...
class AccountSettings {
    ...
    companion object {
        ...
        @JvmStatic
        private var downloadedImageUri: Uri? = null
            ...
            if (it.action == MediaStore.ACTION_IMAGE_CAPTURE) {
                it.extras!!.getUri(MediaStore.EXTRA_OUTPUT)!!.run {
                    mActivityScenarioRule.scenario.onActivity { activity ->
                        ...
                        val inStream =
                            activity.contentResolver.openInputStream(downloadedImageUri!!)
    ...
    fun c2_changeProfileImage_FromGallery() {
        ...
        intending(hasAction(MediaStore.ACTION_PICK_IMAGES))
            .respondWith(Instrumentation.ActivityResult(Activity.RESULT_OK, Intent().setData(downloadedImageUri)))
    ...
    fun l1_setPassword_ValidPassword() {
        ...
        onView(withId(R.id.system_change_password_title)).check(
            matches(
                withText(
    ...
    fun l3_exportBackup_Password() {
        ...
        val dialogString = InstrumentationRegistry.getInstrumentation().targetContext
            .getString(R.string.export_account_wait_message)
        onView(withText(dialogString)).check(matches(isDisplayed()))
...
//        // Todo: To remove when importBackup_withPassword will be implemented
//        accountCreated = false
//        mActivityScenarioRule.scenario.onActivity {
//            AccountUtils.removeAllAccounts(it.mAccountService)
//        }
    }
    ...
    fun m3_importAccount_noPassword() {
        ...
        // Remove account
        mActivityScenarioRule.scenario.onActivity { activity ->
            AccountUtils.removeAllAccounts(activity.mAccountService)
        }
    }
