...
class SmartListFragment : BaseSupportFragment<SmartListPresenter, SmartListView>(),
    ...
    override fun updateList(
        ...
        val activeChannel = ChannelRepository(requireContext()).activeChannel()
        binding?.apply {
            ...
            if (confsList.adapter == null) {
                confsList.adapter = SmartListAdapter(
                        ConversationFacade.ConversationList(ArrayList(visibleConversations)), this@SmartListFragment, conversationFacade, parentDisposable
                ).apply { mSmartListAdapter = this }

                confsList.setHasFixedSize(true)
            } else {
                mSmartListAdapter?.update(ConversationFacade.ConversationList(ArrayList(visibleConversations)))
            }
