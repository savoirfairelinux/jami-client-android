...
class ConversationFragment : BaseSupportFragment<ConversationPresenter, ConversationView>(),
    ...
    override fun displayContact(conversation: ConversationItemViewModel) {
        ...
        val requestId = peerServicesService.queryPeerServices(path.accountId, peerUri)
        mPeerServicesCheckDisposable = peerServicesService.peerServicesReceived
            .filter { it.requestId == requestId && it.accountId == path.accountId && it.peerId == peerUri }
            .firstElement()
