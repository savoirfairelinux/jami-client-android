...
class ShareMediaViewHolder(itemView: View, parentDisposable: CompositeDisposable): RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(item: ShareItem, maxHeight: Int) {
        disposable.clear()
        ...
        when {
            ...
            item.type.startsWith("video/") -> {
                ...
                if (item.type == "application/pdf" && item.data != null) {
                    ...
                    mediaImage.visibility = View.INVISIBLE
                    disposable.add(Single.fromCallable { BitmapUtils.documentToBitmap(itemView.context, item.data, maxHeight)!! }
...
class ShareMediaAdapter(val mediaList: List<ShareItem>, val maxHeight: Int, val parentDisposable: CompositeDisposable) : Adapter<ShareMediaViewHolder>() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        mPendingIntent?.let { pendingIntent -> pendingIntent.type?.let { type ->
            ...
            if (type.startsWith("text/")) {
                mediaList.add(ShareItem(type, text = pendingIntent.getStringExtra(Intent.EXTRA_TEXT)))
            } else {
                val intentUri = pendingIntent.data
                if (intentUri != null)
                    mediaList.add(ShareItem(type, intentUri))
                val cr = context.contentResolver
                pendingIntent.clipData?.let {  clips ->
                    for (i in 0 until clips.itemCount) {
                        clips.getItemAt(i)?.let { clip ->
                            val mimeType =  clip.uri?.let { cr.getType(it) } ?: clip.intent?.type ?: type
                            if (intentUri != clip.uri)
                                mediaList.add(ShareItem(mimeType, clip.uri, clip.text))
                        }
                    }
                }
            }
            binding.mediaList.adapter = ShareMediaAdapter(mediaList, context.resources.getDimensionPixelSize(R.dimen.share_preview_height), mDisposable)
        }}
        return binding.root
    }
