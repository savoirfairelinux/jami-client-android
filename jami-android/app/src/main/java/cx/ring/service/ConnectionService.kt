...
class ConnectionService : ConnectionService() {
    ...
    private fun buildConnection(request: ConnectionRequest): CallConnection {
        val connection = CallConnection(this, request).apply {
            connectionCapabilities = getCapabilities()
            connectionProperties = getProperties()
        }
        (deviceRuntimeService as DeviceRuntimeServiceImpl).addConnection(connection)
        return connection
    }
