                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                    ...
                    disposable.add(Observable
                        ...
                        .subscribe { wantedState ->
                            // Resolve the wanted route against the now-known endpoint list.
                            // This fires when the endpoint list changes, not on every
                            // current-endpoint change, so a system-initiated switch (e.g. a
                            // Bluetooth device connecting mid-call) is honored instead of being
                            // overridden, which would cause repeated switches between sources.
                            ...
                            if (pendingEndpointId == endpointKey) return@subscribe
                            pendingEndpointId = endpointKey
                            requestCallEndpointChange(
                                endpoint,
                                service.mainExecutor,
                                object : OutcomeReceiver<Void, CallEndpointException> {
                                    override fun onResult(result: Void?) {
                                        if (pendingEndpointId == endpointKey) pendingEndpointId = null
