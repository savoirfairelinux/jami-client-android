...
class ChannelRepository(context: Context, accountId: String? = null) {
    ...
    private val keySuffix = accountId?.takeIf { it.isNotBlank() } ?: DEFAULT_ACCOUNT
    ...
    fun load(): List<Channel> {
        ...
        val migrateLegacy = !preferences.getBoolean(LEGACY_MIGRATED_KEY, false)
        ...
        if (accountStored == null || channels != parsed) {
            ...
            if (legacyStored != null)
                preferences.edit().putBoolean(LEGACY_MIGRATED_KEY, true).apply()
        }
        return channels
    }
    ...
    var activeChannelName: String
        get() {
            val raw = preferences.getString(activeKey, null)
                ?: if (!preferences.getBoolean(LEGACY_MIGRATED_KEY, false))
