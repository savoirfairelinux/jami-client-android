...
class PeerServicesViewModel @Inject constructor(
    ...
    private val _copyUrl = MutableSharedFlow<String>(extraBufferCapacity = 4)
    val copyUrl: SharedFlow<String> = _copyUrl.asSharedFlow()
        ...
        disposables.add(
            peerServicesService.tunnelOpened
                ...
                .subscribe { event ->
                    ...
                    // Auto-launch browser for http/https; auto-copy endpoint for others.
                    val scheme = service.scheme.lowercase()
                    val localUrl = "${service.scheme.ifEmpty { scheme }}://127.0.0.1:${event.localPort}"
                    if (scheme == "http" || scheme == "https") {
                        _launchUrl.tryEmit(localUrl)
                    } else {
                        _copyUrl.tryEmit(localUrl)
                    }
                }
        )
