...
class TVAccountExportStep2Fragment : GuidedStepSupportFragment() {
    ...
    override fun onGuidedActionClicked(action: GuidedAction) {
        ...
        when (action.id) {
            GuidedAction.ACTION_ID_CONTINUE -> {
                Log.i(TAG, "User confirmed export (Continue pressed)")
                callback.onIdentityConfirmation(true)
            }
            GuidedAction.ACTION_ID_CANCEL -> {
                Log.i(TAG, "User canceled export (Cancel pressed)")
    ...
    fun update(peerAddress: String?) {
        ...
        Log.d(TAG, "update: peerAddress=$peerAddress, isPasswordMode=$isPasswordMode, " +
                "isCurrentlyPasswordMode=$isCurrentlyPasswordMode, isCurrentlyIPMode=$isCurrentlyIPMode")

        if (isPasswordMode && !isCurrentlyPasswordMode) {
            Log.i(TAG, "Switching UI to Password mode")
            actions.clear()
            setupPasswordModeActions()
        } else if (!isPasswordMode && !isCurrentlyIPMode) {
            Log.i(TAG, "Switching UI to IP mode")
            actions.clear()
            setupIPModeActions(peerAddress)
        } else if (!isPasswordMode && isCurrentlyIPMode) {
            Log.i(TAG, "Updating IP address in UI")
