...
class TVAccountWizard : BaseActivity<AccountWizardPresenter>(), AccountWizardView {
    ...
    fun createAccount() {
        ...
        if (!model.managementServer.isNullOrEmpty()) {
             presenter.initJamiAccountConnect(model, getText(R.string.ring_account_default_name)
                 .toString())
             mJamsAccount = true
        } else if (model.isLink) {
            presenter.initJamiAccountLink(model, getText(R.string.ring_account_default_name)
	        .toString())
            mJamsAccount = false
        } else {
            presenter.initJamiAccountCreation(model, getText(R.string.ring_account_default_name)
	        .toString())
            mJamsAccount = false
        }
    }
