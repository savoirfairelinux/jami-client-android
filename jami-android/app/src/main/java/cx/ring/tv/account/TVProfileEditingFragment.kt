        ...
        registerForActivityResult(ActivityResultContracts.PickVisualMedia()) { uri ->
            if (uri != null) {
                AndroidFileUtils.loadBitmap(requireContext(), uri)
                    .subscribe({ bitmap ->
                        val base64Image: String? = convertProfileImageToBase64(bitmap)
                        if (base64Image != null) {
                            presenter.updateProfile(getCurrentUsername(),
                                base64Image, "image", 1)
                        } else {
                            presenter.updateProfile(getCurrentUsername(), "",
                                "", 2)
            ...
            ProfileCreationFragment.REQUEST_CODE_PHOTO -> if (resultCode == Activity.RESULT_OK && data != null) {
                ...
                if (uri != null) {
                    ...
                    bitmap?.let { source ->
                        ...
                        if (base64Image != null) {
                            presenter.updateProfile(getCurrentUsername(), base64Image,
                                "image", 1)
                        } else {
                            presenter.updateProfile(getCurrentUsername(), "",
                                "", 2)
