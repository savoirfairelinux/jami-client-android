...
class TVAccountExportStep1Fragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.connect.setOnClickListener {
            ...
            (requireActivity() as OnInputCallback).onAuthenticationUri(binding.code.text.toString())
        }
    }
    ...
    fun showError(error: ExportSideInputError?) {
        Log.w(TAG, "Showing error: $error in $currentMode mode")
    ...
    private fun showInputQr(loading: Boolean = false, error: ExportSideInputError? = null) {
        ...
        if (loading) {
            ...
            binding.errorQr.text = when (error) {
                ExportSideInputError.INVALID_INPUT -> getString(R.string.export_side_step1_error_malformed)
            }
