...
@AndroidEntryPoint
class TVJamiAccountConnectFragment : JamiGuidedStepFragment<JamiAccountConnectPresenter,
    JamiConnectAccountView>(), JamiConnectAccountView {
    ...
    override fun onCreateGuidance(savedInstanceState: Bundle?): Guidance {
        ...
        val breadcrumb = ""
        val description = """
            ${getString(R.string.help_credentials_enter)}
            """.trimIndent()
    ...
    override fun onGuidedActionEditedAndProceed(action: GuidedAction): Long {
        when (action.id) {
            SERVER -> {
                action.title = action.editTitle.toString().ifEmpty { getString(R.string.prompt_server) }
                presenter.serverChanged(action.editTitle.toString())
            }
            USERNAME -> {
                action.title = action.editTitle.toString().ifEmpty {
                    getString(R.string.account_username_label) }
                presenter.usernameChanged(action.editTitle.toString())
            }
            PASSWORD -> {
                val description = action.editDescription.toString()
                if (description.isNotEmpty()) {
                    action.description = toPassword(description)
                } else {
                    action.description = getString(R.string.account_enter_password)
                }
    ...
    override fun onCreateActions(actions: MutableList<GuidedAction>, savedInstanceState: Bundle?) {
        ...
        addEditTextAction(context, actions, SERVER, R.string.prompt_server, R.string.account_enter_servername)
        addEditTextAction(context, actions, USERNAME, R.string.account_username_label, R.string.account_enter_username)
        addPasswordAction(context, actions, PASSWORD, getString(R.string.account_enter_password), "", "")
        addDisabledAction(context, actions, CONNECT, getString(R.string.account_connect_button), "", null, true)
    }
