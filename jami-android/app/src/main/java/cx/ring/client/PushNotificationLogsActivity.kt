...
class PushNotificationLogsActivity : AppCompatActivity() {
    ...
    private fun saveFile() {
        if (logFile.exists()) {
            ...
            Snackbar.make(binding.root, "Log file does not exist.", Snackbar.LENGTH_SHORT).show()
        }
    }

    private fun shareLogs() {
        if (logFile.exists()) {
            val logUri = FileProvider.getUriForFile(this, "cx.ring.file_provider", logFile)
            val sendIntent = Intent(Intent.ACTION_SEND).apply {
            addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            setDataAndType(logUri, contentResolver.getType(logUri))
    ...
    private fun setButtonState(logging: Boolean) {
        binding.startLoggingButton.setText(if (logging) R.string.pref_logs_stop else R.string.pref_logs_start)
        binding.startLoggingButton.setBackgroundColor(ContextCompat.getColor(this, if (logging) R.color.red_400 else R.color.colorSecondary))
    }
