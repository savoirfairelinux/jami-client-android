...
class CollabEditorActivity : AppCompatActivity() {
    ...
    private fun embed(bytes: Map<String, ByteArray>, text: String, scheme: String): String {
        ...
        for ((attachmentId, data) in bytes) {
            ...
            written = written.replace(reference(attachmentId, scheme), dataUrl)
        }
        return written
    }
     ...
     * its attachment id under the scheme it drew for this export.
     */
    private fun reference(attachmentId: String, scheme: String): String {
        val unreserved = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_.!~*'()"
        val escaped = StringBuilder(scheme)
