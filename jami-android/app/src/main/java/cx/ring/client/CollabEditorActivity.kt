...
class CollabEditorActivity : AppCompatActivity() {
    ...
    override fun onPrepareOptionsMenu(menu: Menu): Boolean {
        ...
        menu.findItem(R.id.menu_collab_rename)?.isVisible = canRename
    ...
    private fun refreshRenameAuthorization() {
        val self = accountService.getAccount(path.accountId)?.uri
        if (self.isNullOrEmpty()) return
        disposable.add(collaborationService
            .documents(path.accountId, path.conversationUri)
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ documents ->
                canRename = documents.any { it.id == documentId && it.author == self }
                invalidateOptionsMenu()
            }, { e -> Log.w(TAG, "documents", e) }))
    }
