...
class LinkDeviceImportSideActivity : AppCompatActivity(),
    ...
    private fun finish(returnCode: Int = 0) {
        if (returnCode == 0) {
            ...
            Log.e(TAG, "Account importation failed.")
    ...
    private fun launchExitAction() {
        ...
        val message = when (state) { // Todo: move to strings.xml
            is AddDeviceImportState.Connecting, is AddDeviceImportState.Authenticating -> "Exiting now will cancel the account importation process."
            is AddDeviceImportState.InProgress -> "If not too late, exiting now will cancel the account importation process."
