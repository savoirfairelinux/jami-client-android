...
class LinkDeviceExportSideActivity : AppCompatActivity(), ExportSideView,
    ...
    private lateinit var presenter: ExportSidePresenter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        presenter = ExportSidePresenter(this)
    ...
    override fun showResult(result: ExportSideResult) {
        ...
        (binding.viewPager.adapter as ViewPagerAdapter).exportSideStep3.apply {
            when (result) {
                ...
                ExportSideResult.FAILURE -> showError()
            }
        }
    }
    ...
    private fun launchExitAction() {
        ...
        val message = when (state) {
            LinkDeviceState.CONNECTING, LinkDeviceState.AUTHENTICATION -> "Exiting now will cancel the account exportation process."
            LinkDeviceState.IMPORTING -> "If not too late, exiting now will cancel the account exportation process."
