...
class ExportSideStep1Fragment : Fragment() {
    ...
    override fun onAttach(context: Context) {
        ...
        if (requireActivity() is OnInputCallback) {
            _callback = requireActivity() as OnInputCallback
        } else {
            throw RuntimeException("Parent fragment must implement ${OnInputCallback::class.java.simpleName}")
        }
    }
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putString(BUNDLE_KEY_CURRENT_MODE, currentMode.name)
    ...
    override fun onViewStateRestored(savedInstanceState: Bundle?) {
        ...
        savedInstanceState?.let {
            currentMode = InputMode
                .valueOf(it.getString(BUNDLE_KEY_CURRENT_MODE, InputMode.QR.name))
            isLoading = it.getBoolean(BUNDLE_KEY_IS_LOADING, false)
            when (currentMode) {
                InputMode.QR -> showInputQr(loading = isLoading)
                InputMode.CODE -> showInputCode(loading = isLoading)
            }
    ...
    private fun showInputCode(
        ...
        if (loading) {
            ...
            binding.errorCode.visibility = View.INVISIBLE
            binding.connect.isEnabled = false
            binding.code.isEnabled = false
            binding.switchMode.isEnabled = false
        } else if (error != null) {
            binding.errorCode.text = when (error) {
                ExportSideInputError.INVALID_INPUT -> getText(R.string.export_side_step1_error_malformed)
            }
    ...
    companion object {
        ...
        private const val BUNDLE_KEY_CURRENT_MODE = "currentMode"
        private const val BUNDLE_KEY_IS_LOADING = "isLoading"
    }
