    ...
    private fun showInputQr(loading: Boolean = false, error: ExportSideInputError? = null) {
        if (currentMode != InputMode.QR) {
            ...
            binding.switchMode.text =
                getText(R.string.export_side_step1_switch_to_qr)
        }
        if (loading) {
            ...
            binding.errorQr.text = when (error) {
                ...
                ExportSideInputError.NOT_FOUND_ON_NETWORK -> getText(R.string.export_side_step1_error_network)
            }
