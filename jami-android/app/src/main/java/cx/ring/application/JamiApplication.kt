...
abstract class JamiApplication : Application() {
    ...
    fun onPushReceived(isHighPriority: Boolean = true): Boolean {
        ...
        // skip WakeLock, foreground service, and account reactivation entirely.
        // These are informational and will be processed on next natural wake.
        // Check BEFORE touching any push lifecycle state (idle shutdown, counter).
        ...
        synchronized(pushLock) {
            try {
                ...
                // so it doesn't time out before the new push has been handled.
                val existing = pushWakeLock
                if (existing != null) {
                    try { if (existing.isHeld) existing.release() } catch (_: Exception) {}
                }
        ...
        if (isHighPriority && !isInForeground) {
            try {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S
                    ...
                    Log.w(TAG, "Push FGS start not allowed (background restrictions)", e)
                } else {
    ...
    private fun releasePushHold() {
        // If a new push came in while we were waiting, keep the subscription
        // and the WakeLock alive — disposing them here would lose events for
        // the still-in-flight push (VE PS9 comment #4).
        ...
        // Tell the push foreground service it can stop early. We use stopService()
        // instead of startForegroundService(ACTION_PUSH_DONE) because the latter
        // is rejected by the OS on API 31+ when the app is already in background
        // (ForegroundServiceStartNotAllowedException). stopService() has no such
        // restriction and triggers Service.onDestroy() directly; if the service
        // isn't currently running this is a harmless no-op.
        try {
            stopService(Intent(this, PushForegroundService::class.java))
        } catch (e: Exception) {
