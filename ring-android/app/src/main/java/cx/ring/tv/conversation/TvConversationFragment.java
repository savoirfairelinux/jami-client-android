...
public class TvConversationFragment extends BaseSupportFragment<TvConversationPresenter> implements TvConversationView {
        ...
        switch (requestCode) {
            case REQUEST_CODE_PHOTO:
                if (resultCode == Activity.RESULT_OK && data != null) {
                    android.net.Uri image = (android.net.Uri) data.getExtras().get(CustomCameraActivity.EXTRA_DATA);
                    Single<File> file = AndroidFileUtils.getCacheFile(requireContext(), image);
                    createMediaDialog(file);
                }
                break;
    ...
    private void createMediaDialog(Single<File> file) {
        AlertDialog alertDialog = new MaterialAlertDialogBuilder(getContext(), R.style.Theme_MaterialComponents_Dialog)
                .setTitle(R.string.tv_send_media_dialog_message)
                .setMessage("")
