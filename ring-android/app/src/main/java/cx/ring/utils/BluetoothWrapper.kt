...
class BluetoothWrapper(private val mContext: Context, private val btChangesListener: BluetoothChangeListener) {
                    ...
                    if (headsetAdapter != null && status == BluetoothHeadset.STATE_AUDIO_CONNECTED) {
                        ...
                        if (DBG) {
                            for (device in devices)
                                Log.d(TAG, "BluetoothDevice " + device.name + " " + device + " " + device.bondState + " connected: " + headsetAdapter!!.isAudioConnected(device))
                        }
    ...
    val isBTHeadsetConnected: Boolean
        get() = bluetoothAdapter != null && bluetoothAdapter.getProfileConnectionState(BluetoothProfile.HEADSET) == BluetoothAdapter.STATE_CONNECTED
    ...
    fun canBluetooth(): Boolean {
        ...
        val retVal = hasConnectedDevice && audioManager.isBluetoothScoAvailableOffCall
