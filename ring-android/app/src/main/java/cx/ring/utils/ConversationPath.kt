...
class ConversationPath {
    ...
    companion object {
        ...
        fun toUri(interaction: Interaction): android.net.Uri {
            return if (interaction.conversation is Conversation) toUri(
                interaction.account,
                (interaction.conversation as Conversation).uri
            ) else toUri(
                interaction.account,
                Uri.fromString(interaction.conversation.participant)
            )
        }
