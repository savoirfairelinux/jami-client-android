        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch(intent.getAction()) {
                ...
                case ConfigurationManagerCallback.MESSAGE_STATE_CHANGED: {
                    long id = intent.getLongExtra(ConfigurationManagerCallback.MESSAGE_STATE_CHANGED_ID, 0);
                    String status = intent.getStringExtra(ConfigurationManagerCallback.MESSAGE_STATE_CHANGED_STATUS);
                    TextMessage msg = messages.get(id);
                    if (msg != null) {
                        Log.w(TAG, "Message status changed " + id + " " + status);
                        msg.setStatus(status);
                        sendBroadcast(new Intent(ACTION_CONF_UPDATE).putExtra("msg", id));
                    }
                    break;
                }
