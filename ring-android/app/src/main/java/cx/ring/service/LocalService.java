...
*/
    ...
    private class ConversationLoader extends AsyncTask<Void, Void, Map<String, Conversation>> {
        ...
        Map.Entry<HistoryEntry, HistoryCall> findHistoryByCallId(final Map<String, Conversation> confs, String id) {
            for (Conversation c : confs.values()) {
                Map.Entry<HistoryEntry, HistoryCall> h = c.findHistoryByCallId(id);
                if (h != null)
                    return h;
            }
            return null;
        ...
        protected Map<String, Conversation> doInBackground(Void... params) {
            ...
            for (HistoryText htext : historyTexts) {
                ...
                Map.Entry<HistoryEntry, HistoryCall> p = findHistoryByCallId(ret, htext.getCallId());

                if (contact == null && p != null)
                    contact = p.getKey().getContact();
                if (contact == null)
                    continue;
                ...
                if (p  != null) {
                    if (msg.getNumber() == null || msg.getNumber().isEmpty())
                        msg.setNumber(p.getValue().getNumber());
                    p.getKey().addTextMessage(msg);
                }
