...
class DRingService : Service() {
    ...
    private fun parseIntent(intent: Intent) {
        ...
        when (action) {
            ACTION_TRUST_REQUEST_ACCEPT, ACTION_TRUST_REQUEST_REFUSE, ACTION_TRUST_REQUEST_BLOCK -> handleTrustRequestAction(
                intent.data,
                action
            )
            ...
            ACTION_CONV_READ, ACTION_CONV_ACCEPT, ACTION_CONV_DISMISS, ACTION_CONV_REPLY_INLINE -> handleConvAction(
                intent,
                action
            )
            ACTION_FILE_ACCEPT, ACTION_FILE_CANCEL -> if (extras != null) {
                handleFileAction(intent.data, action, extras)
            }
            else -> {
            }
        }
