...
public class CallContact implements Parcelable {
    ...
    public static final Pattern RING_ID_PATTERN = Pattern.compile("^\\s*(?:ring(?:[\\s\\:]+))?(\\p{XDigit}{40})\\s*$", Pattern.CASE_INSENSITIVE);
    ...
    private CallContact(long cID, String k, String displayName, long photoID, ArrayList<Phone> p/*, ArrayList<Phone> sip*/, String mail, boolean user) {
        id = cID;
        key = k;
    ...
    private static String nobracketsNumber(String number) {
...
*/
    ...
    public static class Phone implements Parcelable {
        ...
        public Phone(String num, int cat) {
            //ntype = NumberType.fromInteger(ty);
