...
public class AccountEditionPresenter extends RootPresenter<AccountEditionView> implements Observer<ServiceEvent> {
    ...
    public void update(Observable o, ServiceEvent event) {
        ...
        switch (event.getEventType()) {
            ...
            case ACCOUNTS_CHANGED:
                ...
                getView().displayAccountName(mAccountService.getAccount(mAccountID).getAlias());
    ...
    public void init(String accountId) {
        ...
        if (mAccountID == null && getView() != null) {
            getView().exit();
            return;
        }
        Account selected = mAccountService.getAccount(mAccountID);
        getView().displayAccountName(selected.getAlias());

        if (selected.isRing()) {
            getView().displaySummary(selected);
        }
    }
    ...
    public void prepareOptionsMenu() {
        getView().showAdvancedOption(mAccountService.getAccount(mAccountID).isRing());
        getView().showBlacklistOption(mAccountService.getAccount(mAccountID).isRing());
    }
}
