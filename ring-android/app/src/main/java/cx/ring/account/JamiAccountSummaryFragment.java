...
public class JamiAccountSummaryFragment extends BaseSupportFragment<JamiAccountSummaryPresenter> implements
    ...
    */
    ...
    public void updateUserView() {
        ...
        if (context == null || mAccountService.getCurrentAccount() == null) {
        ...
        mDisposableBag.add(mAccountService
                ...
                .switchMapSingle(account -> AvatarDrawable.load(context, account)
                        .map(avatar -> new Pair<>(account, avatar)))
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(d -> {
                    binding.username.setText(getAccountAlias(d.first));
                    binding.subtitle.setText(getUri(d.first,  getActivity().getString(R.string.account_type_ip2ip)));
    ...
    public void onScrollChanged() {
        if (mIsVisible && binding != null) {
            ((HomeActivity) getActivity()).setToolbarElevation(binding.scrollview.canScrollVertically(SCROLL_DIRECTION_UP));
        }
    }
