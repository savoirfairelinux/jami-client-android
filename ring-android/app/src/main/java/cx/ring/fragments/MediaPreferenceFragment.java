...
public class MediaPreferenceFragment extends PreferenceFragment implements FragmentCompat.OnRequestPermissionsResultCallback{
            ...
            switch (permissions[i]) {
                case Manifest.permission.CAMERA:
                    if (grantResults[i] != PackageManager.PERMISSION_GRANTED) {
                        ...
                        final Account acc = mCallbacks.getAccount();
                        acc.getBasicDetails().setDetailString(AccountDetailBasic.CONFIG_VIDEO_ENABLED, "true");
                        refresh();
    ...
    public void refresh() {
        ...
        if (null != videoCodecsPref && null != videoCodecsPref.listAdapter) {
            videoCodecsPref.listAdapter.notifyDataSetChanged();
        }
    }
}
