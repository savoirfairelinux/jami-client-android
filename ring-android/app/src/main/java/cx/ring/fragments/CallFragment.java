...
public class CallFragment extends BaseSupportFragment<CallPresenter> implements CallView, MediaButtonsHelper.MediaButtonsHelperCallback {
    ...
    public boolean isPermissionAccepted(boolean isIncoming) {
        ...
        if (isIncoming) {
            ...
            REQUEST_PERMISSION = REQUEST_PERMISSION_INCOMING;

        } else {
            audioOnly = args.getBoolean(KEY_AUDIO_ONLY);
            REQUEST_PERMISSION = REQUEST_PERMISSION_OUTGOING;
        }
        if (!audioOnly) {
            ...
            if ((!audioGranted || !videoGranted) &&  android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                requestPermissions(perms.toArray(new String[perms.size()]), REQUEST_PERMISSION);
