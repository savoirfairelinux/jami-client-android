...
class CallFragment() : BaseSupportFragment<CallPresenter, CallView>(), CallView,
    ...
    fun setBottomSheet(display: Boolean) {
        if (display && !presenter.isPipMode) {
            val callActivity = activity as CallActivity?
            val navBarPortraitBottomInset = callActivity?.navBarPortraitBottomInset!!

            val dm = requireContext().resources.displayMetrics
            val orientation = requireContext().resources.configuration.orientation
            val bottomInsets = ViewCompat.getRootWindowInsets(requireView())
                ?.getInsetsIgnoringVisibility(WindowInsetsCompat.Type.navigationBars())?.bottom!!
            val topInsets = ViewCompat.getRootWindowInsets(requireView())
            ...
            var desiredPeekHeight = (((89) * dm.density) + navBarPortraitBottomInset)
            ...
            bottomSheetParams?.let { bs ->
                bs.expandedOffset =
                    ...
                    else (navBarPortraitBottomInset * dm.density).toInt()
        ...
        displayBottomSheet(display && !presenter.isPipMode)
    }
    ...
    override fun resetBottomSheetState() {
        bottomSheetParams?.let { bs ->
            bs.isHideable = false
            bs.state =
                if (bs.state == BottomSheetBehavior.STATE_EXPANDED || bs.state == BottomSheetBehavior.STATE_HALF_EXPANDED) BottomSheetBehavior.STATE_HALF_EXPANDED else BottomSheetBehavior.STATE_COLLAPSED
        }
    }
