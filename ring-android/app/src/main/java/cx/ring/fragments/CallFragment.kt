...
class CallFragment() : BaseSupportFragment<CallPresenter, CallView>(), CallView,
    ...
    fun setBottomSheet(display: Boolean){
        if (display) {
            ...
            val topInsets = ViewCompat.getRootWindowInsets(requireView())?.getInsetsIgnoringVisibility(WindowInsetsCompat.Type.statusBars())?.top!!
            ...
            if ( bottomInsets == 0 || orientation == Configuration.ORIENTATION_LANDSCAPE) {
                ...
                if (gridViewHeight != null) {
                    desiredPeekHeight = (10 * dm.density) + (gridViewHeight / 2) //peekHeight = the height of the fist row of the buttons grid + margintop
            ...
            bottomSheetParams?.let {bs ->
                bs.expandedOffset =
                    if ( bottomInsets == 0 || orientation == Configuration.ORIENTATION_LANDSCAPE) (topInsets * dm.density).toInt()
