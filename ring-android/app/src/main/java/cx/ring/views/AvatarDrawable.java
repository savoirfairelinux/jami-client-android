...
public class AvatarDrawable extends Drawable {
    ...
    public static class Builder {
        ...
        public Builder withViewModel(SmartListViewModel vm) {
            Builder builder = vm.getUri().isSwarm() ? setGroup() : this;
            return (vm.getContact().size() > 2
                    ? builder.withContacts(vm.getContact())
                    : builder.withContact(vm.getContact().isEmpty() ? null : vm.getContact().get(vm.getContact().size() - 1)))
        ...
        if (bitmaps != null) {
            if (cropCircle) {
                ...
                if (backgroundBounds.size() == bitmaps.size())
                    for (int n = 0, s = bitmaps.size(); n < s; i++) {
    ...
    protected void onBoundsChange(Rect bounds) {
        ...
        if (cropCircle) {
            ...
            workspace.set(0, Bitmap.createBitmap(iw, ih, Bitmap.Config.ARGB_8888));
        }
