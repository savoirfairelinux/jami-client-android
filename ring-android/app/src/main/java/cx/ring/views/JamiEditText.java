...
public class JamiEditText extends LinearLayout {
    ...
    public JamiEditText(Context context) {
        this(context, null, 0, 0);
    }

    public JamiEditText(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0, 0);
    }

    public JamiEditText(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }

    public JamiEditText(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        LayoutInflater.from(context).inflate(R.layout.item_jami_edittext, this, true);
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.JamiEditText, defStyleAttr, 0);

        for (int i = 0; i < a.getIndexCount(); i++) {
            int index = a.getIndex(i);

            if (index == R.styleable.JamiEditText_android_text) {
            ...
            if (index == R.styleable.JamiEditText_android_hint) {
                int resourceId = a.getResourceId(index, 0);
                if (0 != resourceId) {
                    try {
                        String string = getResources().getString(resourceId);
            ...
            if (index == R.styleable.JamiEditText_android_tint) {
                setDrawableTint(a.getResourceId(index, 0));
            }
            if (index == R.styleable.JamiEditText_drawable_right) {
                setRightDrawable(a.getResourceId(index, 0));
            }
            if (index == R.styleable.JamiEditText_drawable_left) {
                setLeftDrawable(a.getResourceId(index, 0));
            }
            if (index == R.styleable.JamiEditText_android_enabled) {
                setEnabled(a.getBoolean(index, true));
            }
            if (index == R.styleable.JamiEditText_android_singleLine) {
