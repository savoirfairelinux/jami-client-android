...
class ParticipantsContainerView @JvmOverloads constructor(
    ...
    fun init() {
        ...
        for (i in participants) {
            ...
            val infoOverlayLayoutParams = LayoutParams(
            ...
            if (!i.active) {
                post { participantInfoOverlay.root.layoutParams = infoOverlayLayoutParams }
                if (view == null)
                    toAdd.add(participantInfoOverlay.root)
            } else {
                post {
                    container.layoutParams = infoOverlayLayoutParams
                    participantInfoOverlay.root.layoutParams = params
                }
                if (view == null) {
                    container.addView(participantInfoOverlay.root)
                    toAdd.add(container)
                }
            }
