...
public class AccountWizard extends AppCompatActivity implements Observer<ServiceEvent> {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        String account_to_migrate = null;
        Intent intent = getIntent();
        if (intent != null) {
            mAccountType = intent.getAction();
            Uri path = intent.getData();
            if (path != null)
                account_to_migrate = path.getLastPathSegment();
        }
        if (mAccountType == null)
            mAccountType = AccountConfig.ACCOUNT_TYPE_RING;

        if (savedInstanceState == null) {
            if (account_to_migrate != null) {
            ...
            public void run() {
                ...
                if (!TextUtils.isEmpty(stateAccountId) && stateAccountId.equals(mCreatedAccountId)) {
                    ...
                    if (!mCreationError) {
                        ...
                        mAlertDialog = createAlertDialog(AccountWizard.this, newState);
                        if (mCreatedAccount) {
