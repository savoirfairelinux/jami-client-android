    ...
    public void update(Observable observable, DaemonEvent event) {
        ...
        switch (event.getEventType()) {
            case ACCOUNTS_CHANGED:
                ...
                /*Account createdAccount = mAccountService.getAccount(mCreatedAccountId);
                if (createdAccount!=null && createdAccount.isSip()) {
                    // if we create an SIP account, there will be no REGISTRATION_STATE_CHANGED event
                    // we handle its creation right now
                    handleCreationState(event);
                }*/
                break;
