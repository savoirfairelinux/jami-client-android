...
public class RingtoneActivity extends AppCompatActivity {
    ...
    private List<Ringtone> prepareRingtones() {
        ...
        Drawable ringtoneIcon = getResources().getDrawable(R.drawable.ic_notifications_active_black_24dp);

        if(ringtones == null)
            return ringtoneList;

        ringtoneList.add(new Ringtone("Silent", null, getResources().getDrawable(R.drawable.ic_notifications_off_black_24dp)));

        for(File file : ringtones) {
            String name = stripFileNameExtension(file.getName());
            if(name.equals("AfroNigeria1")) {
                name = "Classic";
            }
            else if(name.equals("AfroNigeria2")) {
                name = "AfroNigeria";
            }
    ...
    private void setDefaultRingtone() {
        ...
        String ringtonePath = new File(ringtonesDir, "AfroNigeria1.opus").getAbsolutePath();
