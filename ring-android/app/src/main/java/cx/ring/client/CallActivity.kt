...
class CallActivity : AppCompatActivity() {
    ...
    private fun handleNewIntent(intent: Intent) {
        ...
        when(action){
            ...
            Intent.ACTION_VIEW, ACTION_CALL_ACCEPT -> {
               ...
               Log.w(TAG, "DEBUG handleNewIntent [VIEW] ---------------------- \n" +
                        " intent.action: $action, [currentId]: $currentId vs $confId :[confId] ")
                if (currentId != confId) {
                   Log.w(TAG, "DEBUG handleNewIntent [VIEW] ---------- currentId != confId -------- \n")
    ...
    private fun moveBottomSheet(movement: BottomSheetAnimation) {
        ...
        mMainView?.let { mainView ->
            ...
            val navbar = ViewCompat.getRootWindowInsets(mainView)
                ?.getInsetsIgnoringVisibility(WindowInsetsCompat.Type.navigationBars()) ?: return
