...
class CallActivity : AppCompatActivity() {
    ...
    var navBarPortraitBottomInset = 0
    ...
    private fun moveBottomSheet(movement: BottomSheetAnimation) {
        ...
        mMainView?.let { mainView ->
            ...
            if (currentOrientation == 1 && bottomInset != 0) navBarPortraitBottomInset = bottomInset

            when (movement) {
                BottomSheetAnimation.UP -> {
                    mainView.findViewById<View>(R.id.call_coordinator_option_container).let {
                        it.updatePadding(bottom = if (currentOrientation != 1) 0 else navBarPortraitBottomInset)
                        mainView.findViewById<View>(R.id.call_options_bottom_sheet)
                            .updatePadding(bottom = if (currentOrientation != 1) ((topInset - 5) * dm.density).toInt() else (navBarPortraitBottomInset * dm.density).toInt())
                        it.animate()
                            .translationY(0f)
                            .alpha(1.0f)
                            .setListener(null)
