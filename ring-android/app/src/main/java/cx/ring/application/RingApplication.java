...
public class RingApplication extends Application {
        ...
        try {
            Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
                ...
                public void uncaughtException(Thread t, Throwable e) {
                    cx.ring.utils.Log.e(TAG, "Uncaught Exception detected in thread ", e);
                    android.os.Process.killProcess(android.os.Process.myPid());
                    System.exit(2);
                }
            });
        } catch (SecurityException e) {
            cx.ring.utils.Log.e(TAG, "Could not set the Default Uncaught Exception Handler", e);
        }
    }
    ...
    private final ServiceConnection mConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName className, IBinder s) {
            cx.ring.utils.Log.d(TAG, "onServiceConnected " + className.getClassName());
            ...
            try {
                ...
                cx.ring.utils.Log.e(TAG, "DRingService start failed", e);
            }
        ...
        public void onServiceDisconnected(ComponentName className) {
            cx.ring.utils.Log.d(TAG, "onServiceDisconnected " + className.getClassName());
        }
    };
    ...
    public void terminateDaemon() {
        ...
        try {
            ...
            cx.ring.utils.Log.e(TAG, "DRingService stop failed", e);
        }
    }
    ...
    public boolean isConnected() {
        ...
        cx.ring.utils.Log.d(TAG, "ActiveNetworkInfo (Wifi): " + (ni == null ? "null" : ni.toString()));
        boolean isWifiConn = ni != null && ni.isConnected();

        ni = cm.getNetworkInfo(ConnectivityManager.TYPE_MOBILE);
        cx.ring.utils.Log.d(TAG, "ActiveNetworkInfo (mobile): " + (ni == null ? "null" : ni.toString()));
    ...
    public void decodingStarted(String id, String shmPath, int width, int height, boolean isMixer) {
        cx.ring.utils.Log.i(TAG, "DRingService.decodingStarted() " + id + " " + width + "x" + height);
        Shm shm = new Shm();
        shm.id = id;
        shm.path = shmPath;
        shm.w = width;
    ...
    public void decodingStopped(String id) {
        cx.ring.utils.Log.i(TAG, "DRingService.decodingStopped() " + id);
    ...
    public void startVideo(Shm input, SurfaceHolder holder) {
        cx.ring.utils.Log.i(TAG, "DRingService.startVideo() " + input.id);

        input.window = mHardwareService.startVideo(input.id, holder.getSurface(), input.w, input.h);

        if (input.window == 0) {
            cx.ring.utils.Log.i(TAG, "DRingService.startVideo() no window ! " + input.id);
    ...
    public void stopVideo(Shm input) {
        cx.ring.utils.Log.i(TAG, "DRingService.stopVideo() " + input.id);
        if (input.window != 0) {
    ...
    public void startCapture(final VideoParams videoParams) {
        ...
        if (surface == null) {
            cx.ring.utils.Log.w(TAG, "Can't start capture: no surface registered.");
        ...
        if (videoParams == null) {
            cx.ring.utils.Log.w(TAG, "startCapture: no video parameters ");
            return;
        }
        cx.ring.utils.Log.d(TAG, "startCapture " + videoParams.id);
        ...
        try {
            ...
            cx.ring.utils.Log.e(TAG, e.getMessage());
            return;
        }
        ...
        try {
            ...
            cx.ring.utils.Log.e(TAG, "Error while settings preview parameters", e);
        }
        ...
        preview.setErrorCallback(new Camera.ErrorCallback() {
            ...
            public void onError(int error, Camera cam) {
                cx.ring.utils.Log.w(TAG, "Camera onError " + error);
                if (preview == cam) {
                    stopCapture();
                }
            }
    ...
    public void stopCapture() {
        cx.ring.utils.Log.d(TAG, "stopCapture " + previewCamera);
