...
public class NotificationServiceImpl extends NotificationService implements Observer<ServiceEvent> {
    ...
    public void cancel(String address) {
        int notificationId = splitAddress(address);
    ...
    public void update(Observable observable, ServiceEvent arg) {
        if (observable instanceof AccountService && arg != null) {
            if (ServiceEvent.EventType.REGISTERED_NAME_FOUND.equals(arg.getEventType())) {
                ...
                // Retrieve previous builder if it exists
                NotificationCompat.Builder messageNotificationBuilder = mNotificationBuilders.get(splitAddress(address));
                //state 0: name found
                if (messageNotificationBuilder != null && state == 0) {
                    ...
                    notificationManager.notify(address.hashCode(), messageNotificationBuilder.build());
                }
            }
        }
    }
