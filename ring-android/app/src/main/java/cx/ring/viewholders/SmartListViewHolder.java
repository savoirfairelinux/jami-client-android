...
public class SmartListViewHolder extends RecyclerView.ViewHolder {
    ...
    public void bind(final SmartListListeners clickListener, final SmartListViewModel smartListViewModel) {
        ...
        if (binding != null) {
            ...
            if (selected != null) {
                compositeDisposable.add(smartListViewModel.getSelected().subscribe(isSelected -> {
                    Log.w("SmartListViewHolder", "selected " + isSelected + " " + smartListViewModel.getContactName());
                    binding.itemLayout.setActivated(isSelected);
                }));
            }
