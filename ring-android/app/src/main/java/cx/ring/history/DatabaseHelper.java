    ...
    private void updateDatabaseFrom8(SQLiteDatabase db) throws SQLiteException {
        if (db != null && db.isOpen()) {
            try {
                ...
                if (callTable.moveToFirst()) {
                    ...
                    while (!callTable.isAfterLast()) {
                        ...
                        if (conversationTable.getCount() == 0) {
                            ...
                            db.update("historycall", contentValues, "accountID=? AND number=?" + contactId,
                ...
                if (textTable.moveToFirst()) {
                    ...
                    while (!textTable.isAfterLast()) {
                        ...
                        if (conversationTable.getCount() == 0) {
                            ...
                            contentValues.clear();
                            contentValues.put("conversationID", id);
                            db.update("historytext", contentValues, "accountID=? AND number=?" + contactId,
                                    new String[]{accountId, contactId});
                        }
                        conversationTable.close();
                ...
                /*Cursor conversationTable = db.rawQuery("SELECT id, accountID, contactID FROM conversation;", new String[]{});
                if(conversationTable.moveToFirst()) {
                    int indexId = conversationTable.getColumnIndex("id");
                    int indexAccountId = conversationTable.getColumnIndex("accountID");
                    int indexContactId = conversationTable.getColumnIndex("contactID");
                    while (!conversationTable.isAfterLast()){
                        long id = conversationTable.getLong(indexId);
                        String accountId = conversationTable.getString(indexAccountId);
                        String contactId = conversationTable.getString(indexContactId);

                        ContentValues contentValues = new ContentValues();
                        contentValues.put("conversationID", id);
                        db.update("historytext", contentValues, "accountID=" + accountId + " AND number=" + contactId,
                                null);
                        db.update("historycall", contentValues, "accountID=" + accountId + " AND number=" + contactId,
                                null);
                        conversationTable.moveToNext();
                    }
                }
                conversationTable.close();*/
