...
public class ConversationAdapter extends RecyclerView.Adapter<ConversationViewHolder> {
    ...
    private SequenceType getMsgSequencing(int i) {
        ...
        if (mInteractions.size() == 1 || i == 0) {
            ...
            if (nextMsg != null) {
                if (isSeqBreak(msg, nextMsg)) {
                    ...
                    return SequenceType.START;
                }
            }
        } else if (mInteractions.size() == i + 1) {
            Interaction prevMsg = getPreviousMessageFromPosition(i);
            if (prevMsg != null) {
                if (isSeqBreak(msg, prevMsg)) {
                    return SequenceType.SINGLE;
                } else {
                    return SequenceType.END;
                }
            }
        }
        Interaction prevMsg = getPreviousMessageFromPosition(i);
        Interaction nextMsg = getNextMessageFromPosition(i);
        SequenceType sequencing = SequenceType.SINGLE;
        if (prevMsg != null && nextMsg != null) {
            if (isSeqBreak(msg, prevMsg) && !isSeqBreak(msg, nextMsg)) {
                sequencing = SequenceType.START;
            } else if (isSeqBreak(msg, nextMsg) && !isSeqBreak(msg, prevMsg)) {
                sequencing = SequenceType.END;
            } else if (!isSeqBreak(msg, nextMsg) && !isSeqBreak(msg, prevMsg)) {
                sequencing = SequenceType.MIDDLE;
            }
        }
        return sequencing;
    }
    ...
    public void onBindViewHolder(@NonNull ConversationViewHolder conversationViewHolder, int position) {
        ...
        Log.d("onBindViewHolder", "position: " + position + " seq: " + interaction.getSequenceType());
    ...
    private void configureForTextMessage(@NonNull final ConversationViewHolder convViewHolder,
        ...
        if (StringUtils.isOnlyEmoji(message)) {
            ...
            switch(getMsgSequencing(position)) {
                case SINGLE:
                ...
                case START:
                    ...
                    if (textMessage.isIncoming()) {
                        convViewHolder.mMsgTxt.setBackground(ContextCompat.getDrawable(context, R.drawable.textmsg_in_first_bg));
                ...
                case MIDDLE:
                    ...
                    if (textMessage.isIncoming()) {
                        convViewHolder.mMsgTxt.setBackground(ContextCompat.getDrawable(context, R.drawable.textmsg_in_middle_bg));
                ...
                case END:
                    ...
                    if (textMessage.isIncoming()) {
                        convViewHolder.mMsgTxt.setBackground(ContextCompat.getDrawable(context, R.drawable.textmsg_in_last_bg));
