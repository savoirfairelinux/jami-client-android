...
std::list<std::unique_ptr<ring::VideoFrame>> frameQueue {};
...
{
    /* On the NDK example, an fread was here. Now we read from frames instead. */
    std::unique_ptr<ring::VideoFrame> frame = std::move(frameQueue.front());
    frameQueue.pop_front();
    size_t bytesRead = frame->size();
    /* TODO: we should likely resize pBufferData here for the data to fit. But can we do that? And just throw away old contents? */
    assert(
        avpicture_layout(
            (AVPicture *)frame->pointer(),
            ring::libav_utils::libav_pixel_format(frame->format()),
            frame->width(),
            frame->height(),
            pBufferData,
            bytesRead
