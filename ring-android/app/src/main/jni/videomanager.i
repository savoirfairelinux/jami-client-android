...
{
    ANativeWindow *window = (ANativeWindow*)((intptr_t) window_);
    __android_log_print(ANDROID_LOG_WARN, "videomanager.i", "setBuffersGeometry %d x %d", width, height);
...
{
    try {
        std::unique_ptr<DRing::FrameBuffer> ret = std::move(windows.at(window));
        if (not ret) {
