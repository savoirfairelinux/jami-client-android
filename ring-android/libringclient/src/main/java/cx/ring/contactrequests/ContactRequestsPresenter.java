    ...
    public void updateList() {
        ...
        mConversationDisposable.add(
                mConversationFacade.getConversationsSubject()
                        .map(account -> {
                            Collection<Conversation> pending = account.getPending();
                            Log.d(TAG, "updateList map " + pending.size());
                            mNotificationService.cancelTrustRequestNotification(account.getAccountID());
