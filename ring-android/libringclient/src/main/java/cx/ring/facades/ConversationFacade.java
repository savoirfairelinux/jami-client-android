...
public class ConversationFacade {
    ...
    public ConversationFacade(HistoryService historyService, CallService callService, AccountService accountService) {
        ...
        mDisposableBag.add(mAccountService
                .getContactAddedSubject()
                .subscribe(conversationHistory ->
                                mAccountService.getAccount(conversationHistory.getAccount()).updateConversationId(conversationHistory),
                        e -> Log.e(TAG, "Error updating conversation")));
    }
    ...
    public Single<TextMessage> sendTextMessage(String account, Conversation c, Uri to, String txt) {
        return mCallService.sendAccountTextMessage(account, to.getRawUriString(), txt)
                .map(id -> {
                    ...
                    TextMessage message = new TextMessage(null, account, Long.toString(id), history, txt);
                    if (c.isVisible())
                        message.read();
    ...
    public void sendTextMessage(Conversation c, Conference conf, String txt) {
        ...
        TextMessage message = new TextMessage(null, c.getAccountId(), conf.getId(), history, txt);
        message.read();
    ...
    public Completable sendFile(String account, Uri to, File file) {
        return Completable.fromAction(() -> {
            ...
            DataTransfer transfer = new DataTransfer(mAccountService.getAccount(account).getByUri(to), account, file.getName(), true, file.length(), 0, 0L);
            // get generated ID
    ...
    private void getSmartlist(final Account account) {
        ...
        mDisposableBag.add(mHistoryService.getSmartlist(account.getAccountID()).subscribeOn(Schedulers.io()).subscribe(conversationHistoryList -> {
    ...
    private Single<Conversation> getConversationHistory(final Account account, final Conversation conversation) {
        ...
        return mHistoryService.getConversationHistory(account.getAccountID(), conversation.getId()).subscribeOn(Schedulers.io()).map(loadedConversation -> {
