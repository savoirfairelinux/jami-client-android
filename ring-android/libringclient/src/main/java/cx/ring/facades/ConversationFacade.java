...
public class ConversationFacade {
    ...
    // private List<Integer> conversationsInHistory = new ArrayList<>();
    ...
    public Single<TextMessage> sendTextMessage(String account, Conversation c, Uri to, String txt) {
        return mCallService.sendAccountTextMessage(account, to.getRawUriString(), txt)
                .map(id -> {
                    ...
                    message.setDaemonId(Long.toString(id));
                    if (c.isVisible())
                        message.read();
                    if (c.getId() == null) {
                        mHistoryService.insertConversation(history, account).subscribe();
                    }
    ...
    public void deleteConversationItem(Interaction element) { // TODO consistency of types...
        switch (element.getType()) {
            case TEXT:
                TextMessage message = (TextMessage) element;
                mDisposableBag.add(Completable.mergeArrayDelayError(
                        mHistoryService.deleteInteraction(message.getId(), message.getAccount()).subscribeOn(Schedulers.io()))
    ...
    public void loadConversationHistory(final Account account, final Conversation conversation, final long offset) {
        // conversationsInHistory.contains(conversation.getId())
        if (conversation.isLoaded()) {
            Log.d(TAG, "loadConversationHistory(): Already loaded");
        } else {
            getConversationHistory(account, conversation, offset);
    ...
    private Disposable getConversationHistory(final Account account, final Conversation conversation, long offset) {
        Log.d(TAG, "getConversationHistory()");
        if (conversation.isLoaded())
            return null;

        return mHistoryService.getConversationHistory(account.getAccountID(), conversation, offset).observeOn(Schedulers.computation()).map(conversationHistories -> {
            ...
            //            / conversationsInHistory.add(conversation.getId());
            return conversation;
    ...
    private void onCallStateChange(SipCall call) {
        ...
        if (conversation != null) {
            conference = conversation.getConference(call.getDaemonIdString()); // daemon id?
            if (conference == null) {
        ...
        if (incomingCall) {
            ...
            mNotificationService.startForegroundService(conference);
            mAccountService.sendProfile(call.getDaemonIdString(), call.getAccount()); // todo daemon id?
            ...
            if (conversation != null && conference.removeParticipant(call)) {
                ...
                if (conversation.getId() == null) {
                    mDisposableBag.add(mHistoryService.insertOrGetConversation(conversation, account.getAccountID()).subscribe((c) -> {
                        call.setConversation(conversation);
                        mHistoryService.insertInteraction(call, account.getAccountID()).subscribe(); // TODO
                    }));
                } else
                    mHistoryService.insertInteraction(call, account.getAccountID()).subscribe(); // TODO
    ...
    public void cancelFileTransfer(long id) {
        ...
        DataTransfer transfer = mAccountService.getDataTransfer(id); // todo
        if (transfer != null)
