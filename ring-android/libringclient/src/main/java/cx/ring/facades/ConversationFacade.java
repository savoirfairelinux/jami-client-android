...
public class ConversationFacade {
    ...
    public ConversationFacade(HistoryService historyService, CallService callService, ContactService contactService, AccountService accountService) {
        ...
        /*mDisposableBag.add(mAccountService.getRegisteredNames()
                ...
                .subscribe(name -> {
                    mAccountService.getAccount(name.accountId).setRingContactName(new Uri(name.address), name.name);
                }));*/
    ...
    public void readMessages(String accountId, Uri contact) {
        ...
        Conversation conversation = account.getByUri(contact);
        if (conversation != null) {
            if (readMessages(conversation)) {
                account.conversationRefreshed();
                mNotificationService.cancelTextNotification(contact);
            }
        }
    }
