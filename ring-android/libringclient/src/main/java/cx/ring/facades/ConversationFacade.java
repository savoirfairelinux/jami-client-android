        ...
        if (observable instanceof HistoryService && event != null) {
            ...
            switch (event.getEventType()) {
                case CALL_STATE_CHANGED:
                    ...
                    if (found == null) {
                        Log.w(TAG, "CALL_STATE_CHANGED : Can't find conference " + callId + " " + newState);
                        break;
                    } else {
                ...
                case INCOMING_CALL:
                    ...
                    Conference conference = getCurrentCallingConf();
                    mDeviceRuntimeService.updateAudioState(conference.isRinging()
                            && conference.isIncoming());
