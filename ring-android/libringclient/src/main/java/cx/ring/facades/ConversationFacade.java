...
public class ConversationFacade {
    ...
    public Completable sendTextMessage(String account, Conversation c, Uri to, String txt) {
        if (c.isSwarm()) {
    ...
    private Observable<SmartListViewModel> observeConversation(Account account, Conversation conversation, boolean hasPresence) {
        ...
        .map(e -> {
            Log.w(TAG, "observeConversation new item " + hasPresence);
            return new SmartListViewModel(conversation, hasPresence);
        });
    ...
    public Single<SipCall> placeCall(String accountId, String number, boolean video) {
        String rawId = Uri.fromString(number).getRawRingId();
        return getAccountSubject(accountId).flatMap(account -> {
            ...
            return mCallService.placeCall(accountId, null, rawId, video);
        });
    }
