...
public class SmartListPresenter extends RootPresenter<SmartListView> implements Observer<ServiceEvent> {
    ...
    private void loadConversations(final String accountId) {
        ...
        compositeDisposable.add(mHistoryService.getConversationsForAccount(accountId)
                .map(new Function<List<ConversationModel>, ArrayList<SmartListViewModel>>() {
                    @Override
                    public ArrayList<SmartListViewModel> apply(@NonNull List<ConversationModel> conversationModels) throws Exception {
                        return modelToViewModel(conversationModels);
                    }
                })
                .subscribeOn(Schedulers.computation())
                .subscribeWith(new ResourceSingleObserver<ArrayList<SmartListViewModel>>() {
                    @Override
                    public void onSuccess(@NonNull ArrayList<SmartListViewModel> smartListViewModels) {
                        if (smartListViewModels.isEmpty()) {
                            getView().setLoading(false);
                            getView().displayNoConversationMessage();
                            getView().hideEmptyList();
                        } else {
                            Collections.sort(smartListViewModels, new SmartListViewModel.SmartListComparator());
                            getView().updateList(smartListViewModels);
                            getView().setLoading(false);
    ...
    private ArrayList<SmartListViewModel> modelToViewModel(List<ConversationModel> conversationModels) throws SQLException {
        ArrayList<SmartListViewModel> smartListViewModels = new ArrayList<>();
        ...
        for (ConversationModel conversationModel : conversationModels) {
            ...
            if (callContact != null) {
                ...
                if (!smartListViewModels.contains(smartListViewModel)) {
