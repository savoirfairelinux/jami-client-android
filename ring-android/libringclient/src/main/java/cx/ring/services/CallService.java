...
//                        Log.i(TAG, "sendFile() thread running... accountId=" + accountId + ", to=" + to + ", filePath=" + filePath);
    ...
    public DataTransferError dataTransferInfo(final Long dataTransferId, final DataTransferInfo dataTransferInfo) {
        Long errorCode = FutureUtils.executeDaemonThreadCallable(
                mExecutor,
                mDeviceRuntimeService.provideDaemonThreadId(),
                true,
                new Callable<Long>() {
                    ...
                    public Long call() throws Exception {
                        ...
                        return Ringservice.dataTransferInfo(dataTransferId, dataTransferInfo);
                    }
                }
        );
        return getDataTransferError(errorCode);
