    ...
    public DataTransferError sendFile(final DataTransfer dataTransfer, File file) {
        ...
        Long errorCode = FutureUtils.executeDaemonThreadCallable(
                ...
                () -> {
                    ...
                    long[] ids = new long[1];
                    long error = Ringservice.sendFile(dataTransferInfo, ids);
                    long newId = ids[0];
                    Log.w(TAG, "sendFile() got daemon ID:" + newId);
                    dataTransfer.setDataTransferId(newId);
                    //mDataTransfers.put(newId, dataTransfer);
                    return error;
                }
        );
