...
public class AccountService extends Observable {
    ...
    private String mVCardDefaultName;

    private ConfigurationCallback mCallbackHandler;

    public AccountService(DaemonService daemonService) {
        mCallbackHandler = new ConfigurationCallbackHandler();
        mAppPath = daemonService.provideFilesDir();
        mVCardDefaultName = daemonService.provideDefaultVCardName();
    }
    ...
    public void sendProfile(final String callId, final String accountId) {
        mExecutor.submit(new Runnable() {
            ...
            public void run() {
                ...
                VCard vcard = VCardUtils.loadLocalProfileFromDisk(mAppPath, accountId, mVCardDefaultName);
