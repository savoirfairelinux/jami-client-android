...
public abstract class HistoryService {
    ...
    public Completable clearHistory(List<Account> accounts) {
        return Completable.fromAction(() -> {
            for (Account account : accounts) {
                ...
                getInteractionDataDao(accountId).executeRaw("DELETE FROM interactions WHERE interactions.type != 'CONTACT'");
            }
        }).subscribeOn(scheduler);
    }
    ...
    Single<Integer> insertConversationAndEvent(ConversationHistory conversation, Interaction event, String accountId) {
        return Single.fromCallable(() -> {
            ...
            if (history == null) {
                event.setConversation(new ConversationHistory(conversationDataDao.extractId(conversationDataDao.createIfNotExists(conversation)), conversation.getParticipant()));
            } else
