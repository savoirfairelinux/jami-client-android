...
public abstract class HistoryService {
    ...
    /**
     * Inserts a conversation and an interaction element. Typically used for the initial contact event.
     *
     ...
     */
    Completable insertAndGetConversation(ConversationHistory conversation, Interaction event, String accountId) {
        return Completable.fromAction(() -> {
            ...
            if (list == null || list.isEmpty()) {
                ...
                event.setConversation(list.get(0));
    ...
    public Single<List<Interaction>> getSmartlist(final String accountId) {
        return Single.fromCallable(() -> {
            QueryBuilder<ConversationHistory, Integer> conversationQueryBuilder = getConversationDataDao(accountId)
                    .queryBuilder();

            PreparedQuery<Interaction> interactionQueryBuilder = getInteractionDataDao(accountId)
                    .queryBuilder()
                    .distinct()
                    .groupBy(Interaction.COLUMN_CONVERSATION)
                    .join(conversationQueryBuilder)
                    .prepare();

            return getInteractionDataDao(accountId)
                    .query(interactionQueryBuilder);
        }).doOnError(e -> Log.e(TAG, "Can't load smartlist from database", e))
                .onErrorReturn(e -> new ArrayList<>());
    }

    /**
     * Retrieves a specific conversation object and creates a new one if it does not exist
     *
     * @param accountId the user's account id
     * @param contact   the conversation participant
     * @return the conversationhistory object
     */
    public Single<ConversationHistory> getConversation(final String accountId, final String contact) {
        return Single.fromCallable(() -> {
            ...
            if (historyList == null || historyList.isEmpty()) {
                ...
                Log.e(TAG, "Can't find conversation", e)).onErrorReturn(e -> null);
    }
