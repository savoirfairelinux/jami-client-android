...
class ConversationPresenter @Inject constructor(
    ...
    fun resume(isBubble: Boolean) {
        ...
        mVisibilityDisposable.add(mConversationSubject
            .subscribe({ conversation: Conversation ->
                ...
                mConversationFacade.readMessages(mAccountService.getAccount(conversation.accountId), conversation, !isBubble)
    ...
    private fun initView(account: Account, c: Conversation, view: ConversationView) {
        ...
        if (mConversationDisposable == null) {
            mConversationDisposable = CompositeDisposable()
        ...
        mConversationDisposable!!.add(c.sortedHistory
            .observeOn(mUiScheduler)
        ...
        mConversationDisposable!!.add(c.cleared
            .observeOn(mUiScheduler)
            .subscribe({ conversation: List<Interaction?>? -> view.refreshView(conversation) }) { e: Throwable? ->
                Log.e(
                    TAG,
        ...
        mConversationDisposable!!.add(c.contactUpdates
            .switchMap { contacts: List<Contact> ->
                Observable.merge(mContactService.observeLoadedContact(c.accountId, contacts, true))
            }
            .observeOn(mUiScheduler)
            .subscribe { contact: Contact ->
                getView()?.updateContact(contact)
            })
        mConversationDisposable!!.add(c.updatedElements
            .observeOn(mUiScheduler)
        ...
        if (showTypingIndicator()) {
            mConversationDisposable!!.add(c.composingStatus
                .observeOn(mUiScheduler)
        ...
        mConversationDisposable!!.add(c.calls
            .observeOn(mUiScheduler)
            .subscribe({ calls: List<Conference> -> updateOngoingCallView(mConversation) }) { e: Throwable? ->
    ...
    fun sendTextMessage(message: String?) {
        if (isEmpty(message) || mConversation == null) {
            return
        }
    ...
    fun saveFile(interaction: Interaction?) {
        ...
        val fileAbsolutePath = deviceRuntimeService.getConversationPath(transfer)
            .absolutePath
    ...
    private fun sendTrustRequest() {
        val contact = mConversation!!.contact
        if (contact != null) {
            contact.status = Contact.Status.REQUEST_SENT
        }
        mVCardService.loadSmallVCardWithDefault(mConversation!!.accountId, VCardService.MAX_SIZE_REQUEST)
            .subscribeOn(Schedulers.computation())
            .subscribe(
                { vCard: VCard? ->
                    mAccountService.sendTrustRequest(
                        mConversation!!, contact!!.uri, Blob.fromString(
                            vcardToString(vCard)
                        )
                    )
                }
                ...
                mAccountService.sendTrustRequest(
                    mConversation!!, contact!!.uri, null
                )
            }
    }

    fun clickOnGoingPane() {
        val conf = if (mConversation == null) null else mConversation!!.currentCall
        if (conf != null) {
    ...
    fun onBlockIncomingContactRequest() {
        mConversationFacade.discardRequest(mConversation!!.accountId, mConversationUri)
        mAccountService.removeContact(mConversation!!.accountId, mConversationUri!!.host, true)
        view?.goToHome()
    }
    ...
    fun onAcceptIncomingContactRequest() {
        mConversationFacade.acceptRequest(mConversation!!.accountId, mConversationUri)
        view?.switchToConversationView()
    }
    ...
    fun onComposingChanged(hasMessage: Boolean) {
        ...
        mConversationFacade.setIsComposing(mConversation!!.accountId, mConversationUri, hasMessage)
    }
