...
public class AccountService {
                ...
                if (account.isSip()) {
                    ...
                    for (Map<String, String> requestData : JamiService.getConversationRequests(account.getAccountID()).toNative()) {
                        ...
                        if (request == null || !conversationId.equals(request.getConversationId())) {
                            ...
                            account.addRequest(new TrustRequest(account.getAccountID(), from, Long.decode(received), null, conversationId));
                        }
                    }
    ...
    void contactAdded(String accountId, String uri, boolean confirmed) {
        Account account = getAccount(accountId);
        if (account != null) {
            Map<String, String> details = JamiService.getContactDetails(accountId, uri);
            Contact contact = account.addContact(details);
            ...
            if (conversationUri.isSwarm())  {
                ...
                if (conversation == null) {
                    conversation = account.newSwarm(conversationUri.getRawRingId(), Conversation.Mode.Syncing);
                    conversation.addContact(contact);
                }
            }
    ...
    public void conversationRequestReceived(String accountId, String conversationId, Map<String, String> metadata) {
        ...
        if (request == null || !conversationId.equals(request.getConversationId())) {
            ...
            account.addRequest(new TrustRequest(account.getAccountID(), contactUri, Long.decode(received), null, conversationId));
        }
    }
