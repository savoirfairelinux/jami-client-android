...
class Account(
    ...
    class ContactLocationEntry {
        ...
        @JvmField
        var location: Observable<ContactLocation>? = null
    }
    ...
    fun onLocationUpdate(location: AccountService.Location): Long {
        ...
        when (location.type) {
            AccountService.Location.Type.Position -> {
                val cl = ContactLocation()
                cl.timestamp = location.date
                cl.latitude = location.latitude
                cl.longitude = location.longitude
                cl.receivedDate = Date()
                ...
                if (ls == null) {
                    ...
                    val entry = ContactLocationEntry()
                    entry.contact = contact
                    entry.location = ls
                    mLocationStartedSubject.onNext(entry)
                } else {
                    if (ls.blockingFirst().timestamp < cl.timestamp) (ls as Subject<ContactLocation>).onNext(cl)
                }
            }
