...
class CallPresenter @Inject constructor(
    ...
    private fun prepareBottomSheetButtonsStatus(conference: Conference) {
        ...
        view?.updateBottomSheetButtonStatus(isSpeakerOn, conference.isAudioMuted, hasMultipleCamera, canDial, showPluginBtn, mOnGoingCall, hasActiveVideo)
    }
    ...
    private fun confUpdate(call: Conference) {
        Log.w(CallPresenter.TAG, "DEBUG confUpdate ---------------- >>  ")
        mConference = call
        Log.w(TAG, "DEBUG confUpdate ----------> callId:${call.id} " + call.state)
        for (i in call.firstCall?.mediaList!!){
            Log.w(TAG, "DEBUG confUpdate ----------> media:  $i")
        }
