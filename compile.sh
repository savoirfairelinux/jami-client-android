...
if [[ $RELEASE -eq 1 ]]; then
    if [ -z "$RING_BUILD_FIREBASE" ]; then
        ...
        cd $TOP && ./gradlew -PbuildFirebase assembleWithFirebaseRelease
    else
        cd $TOP && ./gradlew assembleNoPushRelease
    fi
else
    if [ -z "$RING_BUILD_FIREBASE" ]; then
        echo "Building with Firebase support"
        cd $TOP && ./gradlew -PbuildFirebase assembleWithFirebaseDebug
    else
        cd $TOP && ./gradlew assembleNoPushDebug
    fi
fi
